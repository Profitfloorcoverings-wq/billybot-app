...
export default function EmailThread({ emailThread }: { emailThread: JobPageData["emailThread"] }) {
  ...
  const filtered = useMemo(() => {
    return emailThread.filter((email) => {
      ...
      if (onlyWithAttachments && (!Array.isArray(email.attachments) || email.attachments.length === 0)) return false;
      return true;
    });
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(["all", "inbound", "outbound"] as Direction[]).map((option) => (
          <button
            key={option}
            type="button"
            className={`btn h-9 px-3 ${direction === option ? "btn-primary" : "btn-secondary"}`}
        ...
        <button
          type="button"
          className={`btn h-9 px-3 ${onlyWithAttachments ? "btn-primary" : "btn-secondary"}`}
...
function EmailCard({ email }: { email: JobPageData["emailThread"][number] }) {
  ...
  const bodyText = email.body_text?.trim() || "No email body available.";

  return (
    <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span className="tag">{email.direction ?? "unknown"}</span>
        <span className="text-xs text-[var(--muted)]">{formatTimestamp(email.received_at)}</span>
      ...
      <div className="space-y-1 text-sm">
        <p><span className="text-[var(--muted)]">From:</span> {email.from_email || "—"}</p>
        <p><span className="text-[var(--muted)]">To:</span> {email.to_emails?.join(", ") || "—"}</p>
        <p><span className="text-[var(--muted)]">Cc:</span> {email.cc_emails?.join(", ") || "—"}</p>
        <p><span className="text-[var(--muted)]">Subject:</span> {email.subject || "(No subject)"}</p>
      </div>
      ...
      <div className={`mt-3 overflow-hidden rounded-xl border border-white/10 p-3 ${expanded ? "max-h-none" : "max-h-44"}`}>
        {sanitized ? (
          <div className="prose prose-invert max-w-none text-sm" dangerouslySetInnerHTML={{ __html: sanitized }} />
        ) : (
          <p className="whitespace-pre-wrap text-sm text-slate-200">{bodyText}</p>
        )}
      </div>
      <button type="button" className="mt-2 text-sm text-blue-300 hover:text-blue-200" onClick={() => setExpanded((v) => !v)}>
