  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 16 }}>
        Upload showcase photos on any job to auto-generate posts. Review and approve them here.
      </p>
      ...
      {loading ? (
        ...
        pendingApproval.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, color: "var(--muted)" }}>
            ...
            <p style={{ fontSize: 13 }}>Upload showcase photos to a job and BillyBot will generate social posts for you.</p>
          </div>
        ) : (
              ...
              >
                ...
                {(p.media_urls ?? []).length > 0 && (
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                    {(p.media_urls ?? []).map((url, i) => (
                      <img key={i} src={url} alt={`Media ${i + 1}`} style={{ width: 120, height: 120, borderRadius: 8, objectFit: "cover", border: "1px solid var(--line)" }} />
                    ))}
                  </div>
                )}
                ...
                {p.visual_url && (
                  <div style={{ marginBottom: 12 }}>
                    <img src={p.visual_url} alt="Generated visual" style={{ width: "100%", maxWidth: 400, borderRadius: 8, border: "1px solid var(--line)" }} />
                  </div>
                )}

                {p.video_url && (
                  <div style={{ marginBottom: 12 }}>
                    <video src={p.video_url} controls style={{ width: "100%", maxWidth: 400, borderRadius: 8, border: "1px solid var(--line)" }} />
                  </div>
                )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {posts.map((p) => (
            ...
            >
              {(p.media_urls ?? []).length > 0 ? (
                <img src={(p.media_urls ?? [])[0]} alt="" style={{ width: 40, height: 40, borderRadius: 6, objectFit: "cover", flexShrink: 0 }} />
              ) : p.visual_url ? (
              ...
              <div style={{ minWidth: 36, textAlign: "center" }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: "var(--accent1)" }}>{PLATFORM_LABELS[p.platform] ?? p.platform}</span>
              </div>
          ...
          {posts.length === 0 && (
            <p style={{ color: "var(--muted)", textAlign: "center", padding: 40 }}>No posts yet. Upload showcase photos to a job to get started.</p>
          )}
        </div>
      )}
          ...
          <div style={{ ...modal, maxWidth: 620 }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--text)" }}>
                {PLATFORM_LABELS[editPost.platform] ?? editPost.platform}
              </h2>
            ...
            {/* Media preview */}
            {(editPost.media_urls ?? []).length > 0 && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                {(editPost.media_urls ?? []).map((url, i) => (
                  <img key={i} src={url} alt={`Media ${i + 1}`} style={{ width: 100, height: 100, borderRadius: 8, objectFit: "cover", border: "1px solid var(--line)" }} />
                ))}
              </div>
            )}
            ...
            {editPost.visual_url && (
              <div style={{ marginBottom: 12 }}>
                ...
                <img src={editPost.visual_url} alt="Generated visual" style={{ width: "100%", maxWidth: 300, borderRadius: 8, border: "1px solid var(--line)" }} />
              </div>
            )}
            {editPost.video_url && (
              <div style={{ marginBottom: 12 }}>
                <label style={labelStyle}>Video</label>
                <video src={editPost.video_url} controls style={{ width: "100%", maxWidth: 300, borderRadius: 8, border: "1px solid var(--line)" }} />
              </div>
            )}
