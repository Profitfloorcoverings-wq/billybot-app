...
export default function ChatPage() {
  ...
  const [pricingMilestones, setPricingMilestones] = useState({
    hasEditedPricingSettings: false,
    hasUploadedPriceList: false,
  });
  ...
  useEffect(() => {
    ...
    async function loadPricingMilestones() {
      try {
        ...
        if (error) {
          console.error("Error loading pricing milestones:", error);
          return;
        }

        if (isMounted) {
          setPricingMilestones({
            hasEditedPricingSettings: Boolean(data?.has_edited_pricing_settings),
            hasUploadedPriceList: Boolean(data?.has_uploaded_price_list),
          });
        }
      } catch (err) {
        console.error("Error loading pricing milestones:", err);
      }
    }

    void loadPricingMilestones();
  ...
  const showStarterBanner =
    !pricingMilestones.hasEditedPricingSettings || !pricingMilestones.hasUploadedPriceList;
        ...
        <div className="starter-banner-wrap">
          <div className="starter-banner">
            ...
            <div className="starter-banner-actions">
              {!pricingMilestones.hasEditedPricingSettings ? (
                <Link href="/pricing" className="starter-banner-btn starter-banner-btn-primary">
                  Pricing Settings
                </Link>
              ) : null}
              {!pricingMilestones.hasUploadedPriceList ? (
