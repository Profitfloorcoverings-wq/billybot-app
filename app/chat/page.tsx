...
export default function ChatPage() {
  ...
  const [hasEditedPricingSettings, setHasEditedPricingSettings] = useState(false);
  const [hasUploadedPriceList, setHasUploadedPriceList] = useState(false);
  ...
  useEffect(() => {
    ...
    async function loadPricingBanner() {
      try {
        ...
        const edited = Boolean(data?.has_edited_pricing_settings);
        const uploaded = Boolean(data?.has_uploaded_price_list);

        if (isMounted) {
          setHasEditedPricingSettings(edited);
          setHasUploadedPriceList(uploaded);
          setShowStarterBanner(!(edited && uploaded));
        }
      } catch (err) {
